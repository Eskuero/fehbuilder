...
class RebspickerMulti {
	constructor(domitem, entries, resizelisteners = [window], scrolllisteners = [window], defaultvalues = []) {
		this.domitem = domitem;
		this.domitem.className = "rebspicker-select";
		// Create the element that always shows
		var preview = document.createElement('div');
		preview.className = "rebspickermulti-preview";
		...
		for (let i = 0; i < defaultvalues.length; i++) {
			if ([...this.options].map(opt => opt.value).includes(defaultvalues[i])) {
				this.value = defaultvalues[i];
			}
		}
	...
	clean() {
		while(this.domitem.children[0].firstChild.lastChild) {
			this.domitem.children[0].firstChild.removeChild(this.domitem.children[0].firstChild.lastChild);
		}
	}
	...
	get selectedOptions() {
		return this.domitem.children[0].firstChild.children;
	}
	get preview() {
		return this.domitem.children[0];
	}
	get arrow() {
		return this.domitem.children[0].lastChild;
	}
	get dropdown() {
		return this.domitem.children[1];
	}
	get realinput() {
		return this.domitem.children[1].firstChild.firstChild;
	}
	get selectionlist() {
		return this.domitem.children[1].children[1];
	}
	get options() {
...
class Rebspicker {
	constructor(domitem, entries, resizelisteners = [window], scrolllisteners = [window], defaultvalue) {
		this.domitem = domitem;
		this.domitem.className = "rebspicker-select";
		// Create the element that always shows
		var preview = document.createElement('div');
		preview.className = "rebspicker-preview";
		preview.addEventListener("mousedown", function() {
			event.preventDefault();
		});
		preview.addEventListener("click", function() {
	...
	set value(value) {
		this.domitem.children[0].firstChild.setAttribute("value", value);
		var stringindex = [...this.options].map(opt => opt.value).indexOf(value)
		this.domitem.children[0].firstChild.innerHTML = [...this.options].map(opt => opt.innerHTML)[stringindex];
	}
	get value() {
		return this.domitem.children[0].firstChild.getAttribute("value");
	}
	get preview() {
		return this.domitem.children[0];
	}
