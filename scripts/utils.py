...
def obtaintrueurl(filename):
	...
	artpage = bs4.BeautifulSoup(requests.get("https://feheroes.fandom.com/wiki/File:" + unidecode.unidecode(filename).replace(" ", "_").replace(":", "").replace("?", "").replace("'", "")).text, 'html.parser')
	try:
